...
<section class="detail">
    <div>
        ...
        <div class="thumbs">
            @foreach($product->images as $image)
                ...
                <button type="button" class="js-variant-image @if($resolvedThumb === $initialResolvedImage || ($loop->first && !$defaultVariant)) active @endif" data-image="{{ $resolvedThumb }}" data-color="{{ trim((string)$image->color_name) }}" title="{{ $image->color_name ?: ($image->alt_text ?: $product->name) }}">
    ...
    <div>
        ...
        <div class="variants">
            <strong>Color: <span id="selected-color">{{ trim($defaultVariant?->color_name ?: $defaultVariant?->name) }}</span></strong>
            <div class="variant-list">
                @foreach($product->activeVariants as $index => $variant)
                    ...
                    <button type="button" class="variant js-variant-item @if($isActive) active @endif" data-variant-id="{{ $variant->id }}" data-color="{{ trim($variant->color_name ?: $variant->name) }}" data-price="৳{{ number_format((float)$variant->effective_price, 0) }}" data-sku="{{ $variant->sku }}" @if($varImg) data-image="{{ $varImg }}" @endif @disabled($variant->stock < 1)>
        ...
        <div class="variants">
            ...
            <div class="variant-list">
                @foreach($colorVariants as $variant)
                    <button type="button" class="variant js-variant-image @if($loop->first) active @endif" data-image="{{ app(\App\Services\ProductImageResolver::class)->resolve($variant->image_path) }}" data-color="{{ trim($variant->color_name) }}">
...
@push('scripts')
<script>
function selectVariant(variantId, syncDialog = true) {
    const button = document.querySelector(`.js-variant-item[data-variant-id="${variantId}"]`);
    ...
    if (colorSpan && button.dataset.color) {
        colorSpan.textContent = button.dataset.color;
    }
    ...
    if (button.dataset.sku) {
        ...
        if (skuEl) skuEl.textContent = button.dataset.sku;
    }
    ...
    const colorLower = (button.dataset.color || '').trim().toLowerCase();
    const btnImg = button.dataset.image;
    let thumbMatched = false;
    document.querySelectorAll('.thumbs .js-variant-image').forEach((thumb) => {
        const thumbColor = (thumb.dataset.color || '').trim().toLowerCase();
        const isMatch = (colorLower && thumbColor && thumbColor === colorLower) || (btnImg && thumb.dataset.image === btnImg);
...
document.addEventListener('click', (event) => {
    const button = event.target.closest('.js-variant-item');
    if (button) {
        selectVariant(button.dataset.variantId, true);
    }
});
...
document.querySelectorAll('.thumbs .js-variant-image').forEach((thumb) => {
    thumb.addEventListener('click', () => {
        ...
        const thumbColor = (thumb.dataset.color || '').trim().toLowerCase();
        if (thumbColor) {
            const matchingVariantBtn = Array.from(document.querySelectorAll('.js-variant-item')).find((btn) => {
                return (btn.dataset.color || '').trim().toLowerCase() === thumbColor;
            });
            if (matchingVariantBtn) {
                selectVariant(matchingVariantBtn.dataset.variantId, true);
            } else {
                const colorSpan = document.getElementById('selected-color');
                if (colorSpan && thumb.dataset.color) colorSpan.textContent = thumb.dataset.color;
            }
        }
    });
});

document.querySelectorAll('.variants .js-variant-image').forEach((button) => {
    button.addEventListener('click', () => {
        ...
        document.querySelectorAll('.thumbs .js-variant-image').forEach((thumb) => {
            const thumbColor = (thumb.dataset.color || '').trim().toLowerCase();
            if (colorLower && thumbColor === colorLower) {
                thumb.classList.add('active');
            } else {
                thumb.classList.remove('active');
            }
...
if (purchaseForm && variantDialog && variantSelect) {
    ...
    document.getElementById('variant-confirm')?.addEventListener('click', () => {
        ...
        selectVariant(variantSelect.value, false);
        variantDialog.close();
