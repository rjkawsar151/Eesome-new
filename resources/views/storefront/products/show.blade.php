...
<section class="detail">
    ...
    <div>
        ...
        <form class="buybox" method="POST" action="{{ route('cart.store') }}">@csrf<input type="hidden" name="product_id" value="{{ $product->id }}"><input type="hidden" id="selected-color-input" name="color" value="{{ trim((string)$colorVariants->first()?->color_name) }}"><input type="number" name="quantity" value="1" min="1" max="{{ $product->available_for_preorder ? 100 : max(1,$product->stock) }}" aria-label="Quantity"><button type="submit">Add to cart</button><button type="submit" name="buy_now" value="1">Buy now</button></form>
        @endif
    </div>
</section>
