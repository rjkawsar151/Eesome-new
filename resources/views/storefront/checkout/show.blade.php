...
<div class="field"><label for="shipping_method">Delivery method</label><select id="shipping_method" name="shipping_method" required>@foreach($shippingMethods as $method)<option value="{{ $method->code }}" @selected(old('shipping_method')===$method->code)>{{ $method->name }} — {{ $method->charge_type === 'free' ? 'Free' : '৳'.number_format((float)$method->base_charge,0) }}</option>@endforeach</select></div>
<div class="field"><label for="payment_method">Payment</label><select id="payment_method" name="payment_method" required>@foreach($paymentMethods as $method)<option value="{{ $method->code }}" @selected(old('payment_method')===$method->code)>{{ $method->name }}</option>@endforeach</select></div>
...
<aside class="panel">@php($subtotal=0)<h2>Your order</h2>@foreach($items as $item)@php($p=is_array($item)?$item['product']:$item->product;$q=is_array($item)?$item['quantity']:$item->quantity;$line=(float)$p->effective_price*$q;$subtotal+=$line) <div class="line"><span>{{ $p->name }} Ã— {{ $q }}</span><strong>à§³{{ number_format($line,0) }}</strong></div>@endforeach<div class="line"><strong>Subtotal</strong><strong>à§³{{ number_format($subtotal,0) }}</strong></div><p style="font-size:.85rem;color:var(--text-muted)">The final total, coupon discount, and delivery charge are recalculated on the server.</p><button class="place" type="submit">Place order</button></aside>
</form></main>
@endsection
