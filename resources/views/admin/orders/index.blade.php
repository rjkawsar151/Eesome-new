...
<form class="toolbar" method="GET"><div class="field"><label>Search</label><input class="input" name="search" value="{{ request('search') }}" placeholder="Order, name, email…"></div><div class="field"><label>Status</label><select class="select" name="order_status"><option value="">All</option>@foreach(['Pending','Confirmed','Processing','Shipped','Delivered','Cancelled'] as $s)<option @selected(request('order_status')===$s)>{{ $s }}</option>@endforeach</select></div><div class="field"><label>Payment</label><select class="select" name="payment_status"><option value="">All</option>@foreach(['Pending','Paid'] as $s)<option @selected(request('payment_status')===$s)>{{ $s }}</option>@endforeach</select></div><button class="btn btn-primary">Filter</button></form>
