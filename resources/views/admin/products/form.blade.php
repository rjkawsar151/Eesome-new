        ...
        <div class="form-grid">
            ...
            <div class="field">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.35rem">
                    <label for="product_sku" style="margin:0">SKU <small class="subtle">(Auto-generated)</small></label>
                    <button type="button" id="btn-generate-sku" class="btn btn-soft btn-sm" style="padding:2px 8px;font-size:0.75rem" title="Generate new SKU">Generate</button>
                </div>
                <input class="input" type="text" name="sku" id="product_sku" value="{{ old('sku',$product->sku) }}" placeholder="e.g. EES-TOTE-1234">
            </div>
            <div class="field">
                <label>Badge</label>
    ...
    <div class="form-section">
        ...
        <div class="form-grid">
            ...
            <div class="field">
                ...
                <input class="input" type="number" min="0" name="stock" id="product_stock" value="{{ old('stock',$product->stock ?? 10) }}" required>
                <small id="stock-preorder-hint" class="subtle" style="display:none;margin-top:0.25rem;color:#b45309">Stock is 0 — product will show as Pre-order only.</small>
            </div>
        </div>
    </div>
...
// Auto SKU & Stock 0 Preorder support
(function(){
    const form = document.querySelector('#save-product')?.closest('form');
    if (!form) return;
    const name = document.getElementById('product_name');
    const sku = document.getElementById('product_sku');
    const btnGen = document.getElementById('btn-generate-sku');
    const stock = document.getElementById('product_stock');
    const preorderHint = document.getElementById('stock-preorder-hint');
    const preorderCheck = form.querySelector('input[name="is_preorder"]');

    function makeSku(val) {
        const clean = String(val || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8) || 'PROD';
        const rnd = Math.floor(1000 + Math.random() * 9000);
        return 'EES-' + clean + '-' + rnd;
    }

    if (sku && !sku.value.trim()) {
        sku.dataset.autoGenerated = 'true';
    }

    if (name && sku) {
        name.addEventListener('input', () => {
            if (!sku.value.trim() || sku.dataset.autoGenerated === 'true') {
                if (name.value.trim()) {
                    sku.value = makeSku(name.value);
                    sku.dataset.autoGenerated = 'true';
                }
            }
        });
        sku.addEventListener('input', () => {
            sku.dataset.autoGenerated = 'false';
        });
    }

    if (btnGen && sku) {
        btnGen.addEventListener('click', () => {
            sku.value = makeSku(name?.value || 'PROD');
            sku.dataset.autoGenerated = 'false';
        });
    }
    ...
    function checkStock() {
        ...
        if (!isNaN(val) && val <= 0) {
            if (preorderHint) preorderHint.style.display = 'block';
            if (preorderCheck) preorderCheck.checked = true;
        } else {
            if (preorderHint) preorderHint.style.display = 'none';
        }
    }

    if (stock) {
        stock.addEventListener('input', checkStock);
        stock.addEventListener('change', checkStock);
        checkStock();
    }
})();
