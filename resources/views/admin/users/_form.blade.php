...
<div class="field"><label for="role">Role</label><select class="select" id="role" name="role" required>@foreach(['customer','manager','content editor','admin','super admin'] as $role)<option value="{{ $role }}" @selected(old('role',$user->role ?? 'customer')===$role)>{{ ucfirst($role) }}</option>@endforeach</select></div>
