...
@foreach($products as $product)<tr><td>{{ $product->name }}</td><td>{{ $product->sku ?: '—' }}</td><td><span class="badge {{ $product->stock <= 5 ? 'badge-red' : 'badge-green' }}">{{ $product->stock }}</span></td><td><form method="POST" action="{{ route('admin.inventory.adjust',$product) }}" style="display:flex;gap:.4rem">@csrf<input class="input" style="width:90px" type="number" name="quantity_delta" required placeholder="+/-"><input class="input" style="width:190px" name="reference" required placeholder="Reason/reference"><button class="btn btn-primary">Adjust</button></form></td></tr>@endforeach
