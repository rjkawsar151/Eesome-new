...
<section class="card"><div style="display:flex;justify-content:space-between"><h2>Recent orders</h2><a href="{{ route('admin.orders.index') }}">View all</a></div><div class="table-wrap"><table class="table"><thead><tr><th>Order</th><th>Customer</th><th>Status</th><th>Total</th><th>Action</th></tr></thead><tbody>@forelse($recentOrders as $order)<tr><td><a href="{{ route('admin.orders.show',$order) }}">{{ $order->order_number ?: '#'.$order->id }}</a></td><td>{{ $order->customer_name }}</td><td><span class="badge">{{ $order->order_status }}</span></td><td>&#2547;{{ number_format((float)$order->total_amount,0) }}</td><td><a class="btn btn-soft btn-sm" href="{{ route('admin.orders.show',$order) }}">View order</a></td></tr>@empty<tr><td colspan="5">No orders yet.</td></tr>@endforelse</tbody></table></div></section>
